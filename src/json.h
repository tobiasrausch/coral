...
namespace sc
{
  ...
  template<typename TConfig, typename THeader, typename TGenomicWC>
  inline void
  scJsonOut(TConfig const& c, THeader const& hdr, TGenomicWC const& gWC) {
	...
	int32_t pos = 0;
	for (uint32_t k = 0; k < gWC[refIndex].size(); ++k) {
	  if (k > 0) rfile << ", ";
	  rfile << pos;
	  pos += c.window;
	}
